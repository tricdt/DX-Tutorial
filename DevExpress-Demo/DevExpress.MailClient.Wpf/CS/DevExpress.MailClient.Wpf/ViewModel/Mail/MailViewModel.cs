...
namespace DevExpress.MailClient.ViewModel {
    ...
    public class MailViewModel : ContentViewModelBase<MailMessageViewModel> {
        ...
        public void SaveMessageOnClosingConfirmation(object parameter) {
            ...
            var result = this.GetService<IMessageBoxService>().ShowMessage("Do you want to save changes?", "DevExpress Mail Client", MessageButton.YesNoCancel);
            if(result == MessageResult.Cancel) {
                if(args != null)
                    args.Cancel = true;
            } else if(result == MessageResult.OK || result == MessageResult.Yes)
        ...
        int GetMessagesUnreadCount(IMailFolderDescription folder) {
            return Items.Cast<MailMessageViewModel>().Count(x => x.IsUnread && (folder.Folder == MessageFolderName.All || x.Folder == folder.Folder) && (x.Type == folder.Type));
        }
