...
namespace DevExpress.DevAV.ViewModels {
    partial class CustomerViewModel {
        ...
        public IEnumerable<object> UpdateMapItems(DateTime start, DateTime end) {
            ...
            var items = Entity.Orders.Where(x => x.OrderDate >= start && x.OrderDate <= end).GroupBy(order => order.Store);
            if(items.Count() > 0) {
                decimal minimumSalesValue = items.Min(orders => orders.CustomSum(order => order.TotalAmount));
                decimal maximumSalesValue = items.Max(orders => orders.CustomSum(order => order.TotalAmount));
